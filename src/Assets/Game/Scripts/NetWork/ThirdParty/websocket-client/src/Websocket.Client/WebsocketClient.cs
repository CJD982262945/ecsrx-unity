...
namespace Websocket.Client
{
    ...
    public class WebsocketClient : IWebsocketClient
    {
        ...
        public void Dispose()
        {
            ...
            {
                Debug.LogError(L($"Failed to dispose client, error: {e.Message}"));
            }
        ...
        public async Task Start()
        {
            if (IsStarted)
            {
                Debug.Log(L("Client already started, ignoring.."));
                return;
            }
            IsStarted = true;

            Debug.Log(L("Starting.."));
        ...
        public async Task Reconnect()
        {
            if (!IsStarted)
            {
                Debug.Log(L("Client not started, ignoring reconnection.."));
                return;
            }
        ...
        private async Task SendTextFromQueue()
        {
            try
            {
                foreach (var message in _messagesTextToSendQueue.GetConsumingEnumerable(_cancellationTotal.Token))
                {
                    ...
                    {
                        Debug.LogError(L($"Failed to send text message: '{message}'. Error: {e.Message}"));
                    }
                }
            }
            ...
            catch (Exception e)
            {
                ...
                Debug.LogError(L($"Sending text thread failed, error: {e.Message}. Creating a new sending thread."));
                StartBackgroundThreadForSendingText();
            }
        ...
        private async Task SendBinaryFromQueue()
        {
            try
            {
                foreach (var message in _messagesBinaryToSendQueue.GetConsumingEnumerable(_cancellationTotal.Token))
                {
                    ...
                    {
                        Debug.LogError(L($"Failed to send binary message: '{message}'. Error: {e.Message}"));
                    }
                }
            }
            catch (TaskCanceledException)
            ...
            catch (Exception e)
            {
                ...
                Debug.LogError(L($"Sending binary thread failed, error: {e.Message}. Creating a new sending thread."));
                StartBackgroundThreadForSendingBinary();
            }
...
#pragma warning restore 4014               
            ...
            catch (Exception e)
            {
                ...
                Debug.LogError(L($"Exception while connecting. " +
        ...
        private async Task Listen(ClientWebSocket client, CancellationToken token)
        {
            ...
            {
                Debug.LogError(L("Error while listening to websocket stream"));
            }
        }
        ...
        private void LastChance(object state)
        {
            ...
            if (diffMs > timeoutMs)
            {
                Debug.Log(L($"Last message received more than {timeoutMs:F} ms ago. Hard restart.."));
